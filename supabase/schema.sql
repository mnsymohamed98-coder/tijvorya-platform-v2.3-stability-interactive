...
-- v3.0 fix: Postgres fires a table's BEFORE INSERT trigger for every
...
-- on to update an existing row. The original INSERT branch below assumed
...
-- resend the store's current (already-true) verified value. Fixed by
-- looking up whether a row with this id already exists and, if so,
-- applying the same "only block actual changes" logic the UPDATE branch
-- already had - regardless of what tg_op claims.
...
-- v3.1: column-immutability guards for products and orders, closing the
-- same class of gap the store/reel triggers above already cover - see
-- migrations/20260831_v3_1_protect_metrics_and_orders.sql for the full
-- rationale.
