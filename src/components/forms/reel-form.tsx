...
export function ReelForm({ reel }: { reel?: Reel }) {
  ...
  const [cover, setCover] = useState(reel?.cover ?? "");
  const [caption, setCaption] = useState(reel?.caption ?? "");
  const [captionEn, setCaptionEn] = useState(reel?.captionEn ?? "");
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Editing preserves every field saveReel/insertReel doesn't itself
      // recompute (views, likes, and the other v1.8 analytics counters) -
      // this is a full-row upsert, so anything left out here would silently
      // reset a live reel's real counts back to zero.
      const nextReel: Reel = {
        ...reel,
        id: reel?.id ?? uid("reel"), storeId: merchantStore.id, productId,
        caption: caption.trim(), captionEn: captionEn.trim() || caption.trim(),
        videoUrl, cover: cover || selectedProduct.image || "/assets/reel-1.png",
        status: requestedStatus, views: reel?.views ?? 0, likes: reel?.likes ?? 0,
        hashtags: hashtags.trim() ? hashtags.trim().split(/\s+/) : strategy?.hashtags,
        bestPostTime: reel?.bestPostTime ?? strategy?.bestPostTime,
        createdAt: reel?.createdAt ?? now, submittedAt: requestedStatus === "pending" ? now : undefined,
      };
      await saveReel(nextReel);
  ...
  return <form className="editor-form reel-editor" onSubmit={submit}>
    <section className="editor-card"><div className="card-head"><div><span className="eyebrow">VIDEO COMMERCE</span><h3>{locale === "ar" ? "ملف الريلز" : "Reel video"}</h3></div>{!reel && <span className="secure-note">{locale === "ar" ? `الحد الأقصى ${platformSettings.maxReelSizeMB}MB` : `${platformSettings.maxReelSizeMB}MB maximum`}</span>}</div>{reel ? <div><div className="admin-video-preview"><PersistentVideo src={reel.videoUrl} poster={reel.cover} controls playsInline preload="metadata" /></div><p className="inline-warning">{locale === "ar" ? "لا يمكن استبدال الفيديو بعد النشر. احذف الريلز وارفع واحدًا جديدًا إذا لزم تغيير المحتوى المرئي." : "The video can't be replaced after upload. Delete this reel and upload a new one if the footage itself needs to change."}</p></div> : <MediaUploader resourceType="video" folder="tijvorya/reels" value={videoUrl} onChange={setVideoUrl} maxMB={platformSettings.maxReelSizeMB} label={locale === "ar" ? "اختر الفيديو من جهازك" : "Choose video from your device"} />}</section>
    <section className="editor-card">
      ...
      <label className="field"><span>{locale === "ar" ? "الوسوم — افصل بمسافة" : "Hashtags — space separated"}</span><input name="hashtags" value={hashtags} onChange={(event) => setHashtags(event.target.value)} placeholder={locale === "ar" ? "#عطور #تسوق" : "#fragrance #shopping"} /></label>
    </section>
    {reel ? <section className="editor-card"><div className="card-head"><div><span className="eyebrow">COVER</span><h3>{locale === "ar" ? "غلاف الفيديو" : "Video cover"}</h3></div></div><p className="inline-warning">{locale === "ar" ? "لا يمكن تغيير الغلاف بعد النشر." : "The cover image can't be changed after upload."}</p></section> : <section className="editor-card"><div className="card-head"><div><span className="eyebrow">COVER</span><h3>{locale === "ar" ? "غلاف الفيديو" : "Video cover"}</h3></div></div><MediaUploader resourceType="image" folder="tijvorya/reels" value={cover} onChange={setCover} maxMB={5} label={locale === "ar" ? "صورة غلاف اختيارية" : "Optional cover image"} /></section>}
