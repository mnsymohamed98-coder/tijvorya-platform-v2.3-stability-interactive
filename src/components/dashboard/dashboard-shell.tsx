...
export function DashboardShell({
  ...
  const ownedStoreIds = new Set(
    stores
      .filter((store) => store.ownerId === currentUser?.id)
      .map((store) => store.id)
  );
  ...
  const messageBadge =
    role === "merchant"
      ? conversations
          .filter((conversation) =>
            ownedStoreIds.has(conversation.storeId)
          )
          .reduce(
            (sum, conversation) =>
              sum + conversation.unreadByMerchant,
            0
          )
  ...
  if (
    ...
    (currentUser.role !== role &&
      !(role === "merchant" && currentUser.role === "influencer"))
  ) {
    return (
      <div className="auth-gate">
        ...
        <div className="auth-gate-card">
          ...
          <p>
            {locale === "ar"
              ? "سجّل الدخول بالحساب المناسب للوصول إلى لوحة التحكم."
              : "Sign in with the appropriate account to access this dashboard."}
          </p>
  ...
  const avatarFallback =
    currentUser.fullName?.trim().slice(0, 2).toUpperCase() || "TJ";
    ...
    <div className="dashboard-layout">
      ...
      >
        ...
        <div className="sidebar-plan">
          <span
            className={`mode-dot ${
              productionMode ? "production" : "demo"
            }`}
          />
          ...
          <p>
            {productionMode
              ? locale === "ar"
                ? "البيانات متصلة بـ Supabase."
                : "Data is connected to Supabase."
              : locale === "ar"
        ...
        <header className="dashboard-topbar">
          ...
          <div className="dashboard-account">
            ...
            <span className="avatar">
              {isAvatarImage(avatarValue) ? (
                <img
                  src={avatarValue}
                  alt={currentUser.fullName}
                />
              ) : (
                avatarValue || avatarFallback
              )}
            </span>
