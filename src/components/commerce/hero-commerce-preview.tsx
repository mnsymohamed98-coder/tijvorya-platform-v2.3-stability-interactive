    ...
    <div className="hero-live-body">
      ...
      <div className="hero-live-content">
        ...
        {mode === "products" && <section className="hero-live-panel" role="tabpanel">
          ...
          {visibleProducts.length > 0 ? <div className="hero-live-product-grid">{visibleProducts.map((product) => <article key={product.id}><div>{product.image ? <PersistentImage className="media-fill" src={product.image} alt={locale === "ar" ? product.name : product.nameEn} /> : <Package />}</div><strong>{locale === "ar" ? product.name : product.nameEn}</strong><span>{formatMoney(product.price, locale)}</span></article>)}</div> : <PreviewEmpty icon={<Package />} text={locale === "ar" ? "ستظهر المنتجات الحقيقية هنا بعد نشرها." : "Real products will appear here after publishing."} />}
        </section>}
        ...
        {mode === "stores" && <section className="hero-live-panel" role="tabpanel">
          ...
          {visibleStores.length > 0 ? <div className="hero-live-store-grid">{visibleStores.map((store) => <article key={store.id}><div><PersistentImage className="media-cover" src={store.logo} alt={locale === "ar" ? store.name : store.nameEn} /></div><span><strong>{locale === "ar" ? store.name : store.nameEn}</strong><small>{store.website?.domain || `${store.slug}.tijvorya.com`}</small></span></article>)}</div> : <PreviewEmpty icon={<StoreIcon />} text={locale === "ar" ? "ستظهر المتاجر الجاهزة هنا تلقائيًا." : "Launch-ready stores will appear here automatically."} />}
        </section>}
        ...
        {mode === "reels" && <section className="hero-live-panel" role="tabpanel">
          ...
          {visibleReels.length > 0 ? <div className="hero-live-reel-grid">{visibleReels.map((reel) => <article key={reel.id}><div><PersistentImage className="media-fill" src={reel.cover} alt={locale === "ar" ? reel.caption : reel.captionEn} /><span><Play /></span></div><strong>{locale === "ar" ? reel.caption : reel.captionEn}</strong></article>)}</div> : <PreviewEmpty icon={<Film />} text={locale === "ar" ? "ستظهر الريلز المعتمدة هنا بعد نشرها." : "Approved reels will appear here once published."} />}
        </section>}
      </div>
    </div>
  </div>;
