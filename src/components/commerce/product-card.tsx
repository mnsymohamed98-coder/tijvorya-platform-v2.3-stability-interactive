...
export function ProductCard({ product, store }: { product: Product; store?: Store }) {
  ...
  const labels = locale === "ar"
    ? { view: `عرض ${productName}`, add: `اشترِ ${productName} الآن`, favorite: isFavorite ? `إزالة ${productName} من المفضلة` : `إضافة ${productName} إلى المفضلة`, verified: "متجر موثّق" }
    : { view: `View ${productName}`, add: `Buy ${productName} now`, favorite: isFavorite ? `Remove ${productName} from favorites` : `Add ${productName} to favorites`, verified: "Verified store" };

  async function buyNow() {
    if (await addToCart(product.id)) router.push(`/${locale}/checkout`);
  }
  ...
  return <article className="product-card">
    ...
    <div className="product-body">
      ...
      <button type="button" className="button button-dark button-block" disabled={product.stock < 1} onClick={buyNow} aria-label={labels.add}><ShoppingBag size={17} />{product.stock > 0 ? (locale === "ar" ? "اشترِ الآن" : "Buy now") : (locale === "ar" ? "نفد المخزون" : "Sold out")}</button>
    </div>
  </article>;
}
