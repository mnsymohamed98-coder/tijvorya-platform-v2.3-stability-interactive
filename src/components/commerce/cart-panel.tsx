...
export function CartPanel({ deliveryFee, hideCheckoutCta }: { deliveryFee?: number; hideCheckoutCta?: boolean } = {}) {
  const { locale, cart, products, updateCartQuantity, removeFromCart, productionMode, mergeProducts } = useApp();
  ...
  const delivery = deliveryFee === undefined ? undefined : Math.max(0, deliveryFee);
  ...
  return <div className="cart-layout">
    ...
    <aside className="order-summary"><h3>{locale === "ar" ? "ملخص الطلب" : "Order summary"}</h3><div><span>{locale === "ar" ? "المجموع الفرعي" : "Subtotal"}</span><strong>{formatMoney(subtotal, locale)}</strong></div><div><span>{locale === "ar" ? "التوصيل" : "Delivery"}</span><strong>{delivery === undefined ? (locale === "ar" ? "يُحدد حسب منطقتك" : "Based on your area") : delivery ? formatMoney(delivery, locale) : (locale === "ar" ? "مجاني" : "Free")}</strong></div><div className="summary-total"><span>{locale === "ar" ? "الإجمالي التقديري" : "Estimated total"}</span><strong>{formatMoney(subtotal + (delivery ?? 0), locale)}</strong></div>{!hideCheckoutCta && <><Link className="button button-dark button-block" href={`/${locale}/checkout`}>{locale === "ar" ? "متابعة إتمام الطلب" : "Continue to checkout"}</Link><small>{locale === "ar" ? "يُعاد التحقق من السعر والمخزون عند التأكيد." : "Price and stock are verified again at confirmation."}</small></>}</aside>
  </div>;
}
