...
export function SiteFooter({ locale }: { locale: "ar" | "en" }) {
  return <footer className="site-footer">
    <div className="container footer-grid">
      ...
      <div><strong>{locale === "ar" ? "الثقة" : "Trust"}</strong><Link href={`/${locale}/privacy`}>{locale === "ar" ? "الخصوصية" : "Privacy"}</Link><Link href={`/${locale}/terms`}>{locale === "ar" ? "الشروط" : "Terms"}</Link><Link href={`/${locale}/login`}>{locale === "ar" ? "تسجيل الدخول" : "Sign in"}</Link><Link href={`/${locale}/admin-access`}>{locale === "ar" ? "دخول الإدارة" : "Admin access"}</Link></div>
    </div>
    <div className="container footer-bottom"><span>© {new Date().getFullYear()} Tijvorya</span><span>{locale === "ar" ? "تجارة اجتماعية موثوقة، مصممة للنمو عبر الأسواق." : "Trusted social commerce, designed to grow across markets."}</span></div>
  </footer>;
}
