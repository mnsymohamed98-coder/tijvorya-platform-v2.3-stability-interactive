...
const allowedFolders = new Set(["tijvorya/products", "tijvorya/reels", "tijvorya/stores", "tijvorya/payments"]);
...
export async function POST(request: Request) {
  try {
    ...
    // Payment-proof screenshots are uploaded from checkout, which (like
    // ordering itself) doesn't require sign-in - guest checkout is a
    // supported flow. Every other folder still needs a real merchant/
    // influencer/admin session, same as before.
    if (process.env.NEXT_PUBLIC_DEMO_MODE !== "true" && folder !== "tijvorya/payments") {
