  ...
  return <PublicShell locale={locale}>
    ...
    <section className="hero-section hero-global"><span className="hero-ambient hero-ambient-a" aria-hidden="true" /><span className="hero-ambient hero-ambient-b" aria-hidden="true" /><span className="hero-ambient hero-ambient-c" aria-hidden="true" /><div className="container hero-grid hero-grid-enhanced">
      <div className="hero-copy">
        ...
        <div className="hero-proof"><div><CheckCircle2 /><strong>{locale === "ar" ? "منتجات مضمونة" : "Guaranteed products"}</strong><span>{locale === "ar" ? "وعروض حصرية" : "And exclusive deals"}</span></div><div><CheckCircle2 /><strong>{locale === "ar" ? "شراء من الريلز" : "Shop from reels"}</strong><span>{locale === "ar" ? "مباشر وسريع" : "Direct and fast"}</span></div><div><CheckCircle2 /><strong>{locale === "ar" ? "شحن سريع" : "Fast shipping"}</strong><span>{locale === "ar" ? "وتتبع مباشر للطلب" : "With live order tracking"}</span></div><div><BadgeCheck /><strong>{locale === "ar" ? "تجربة موثوقة" : "Trusted experience"}</strong><span>{locale === "ar" ? "متاجر ومحتوى تحت المراجعة" : "Stores and content under review"}</span></div><div><PackageCheck /><strong>{locale === "ar" ? "رحلة طلب واضحة" : "Clear order journey"}</strong><span>{locale === "ar" ? "من الاكتشاف حتى متابعة الطلب" : "From discovery to order tracking"}</span></div></div>
      </div>
